...
interface IBillingRepository : IMVPRepository {
    suspend fun createPurchaseIntent(event: EventAbs, teamId: String? = null): Result<PurchaseIntent?>
...
class BillingRepository(
    ...
    override suspend fun createPurchaseIntent(event: EventAbs, teamId: String?): Result<PurchaseIntent?> =
        runCatching {
            ...
            if (user.stripeAccountId.isNullOrBlank()) {
                ...
                Json.decodeFromString(PurchaseIntent.serializer(),
                    functions.createExecution(
                        functionId = DbConstants.BILLING_FUNCTION,
                        body = Json.encodeToString(
                            CreatePurchaseIntent(
                                "create_purchase_intent", user.id, event.id, teamId, isTournament
                            )
                        ),
                        async = false,
                    ).responseBody
    ...
    override suspend fun createAccount(): Result<String> =
        runCatching {
            val user = userRepository.currentUser.value.getOrThrow()
...
private data class CreatePurchaseIntent(
    val command: String, val userId: String, val eventId: String, val teamId: String?, val isTournament: Boolean
)
