...
This plan follows `PLANS.md` at the repository root. The same plan is also stored in the companion `mvp-site` worktree so either repository can be opened independently and still explain the end-to-end change.
...
Revision note, 2026-05-06: Continued implementation in the main checkout on branch `codex/tournament-pool-play` after the temporary worktree was removed. `EventDetailScreen` now shows standings for tournament pool play, filters schedule matches by bracket division plus optional generated pool division, and adds a Pool dropdown to the schedule dock. `EventDetailComponent` now loads and confirms standings for generated pool divisions. Verification passed with `.\gradlew :composeApp:compileDebugKotlinAndroid --no-daemon`.

Revision note, 2026-05-06: Corrected the mobile tournament standings UI so the Standings dock keeps the bracket division selector and adds a required generated pool selector beside it. The selected pool defaults to the first generated pool for the bracket, standings loading and confirmation target that pool division, and local fallback standings filter teams and matches by the selected pool. Verification passed again with `.\gradlew :composeApp:compileDebugKotlinAndroid --no-daemon`.

Revision note, 2026-05-06: Corrected mobile division capacity display for tournament pool play. `DivisionCapacitySummary` now groups generated pool divisions by their mapped bracket division and displays one bracket capacity row using summed pool capacity, so event details no longer list Pool A/B/C as registration capacities. Verification passed with `.\gradlew :composeApp:testDebugUnitTest --tests "com.razumly.mvp.eventDetail.DivisionCapacitySummaryTest" --console=plain --no-daemon`.

Revision note, 2026-05-06: Hardened the mobile pool selectors for events loaded from list/search payloads that do not include explicit `playoffPlacementDivisionIds`. The app now infers tournament bracket ids from generated pool suffixes such as `_pool_a`, uses bracket options plus a separate Pool dock dropdown for schedule and standings, and filters remote standings rows to teams assigned to the selected pool. Verification passed with `.\gradlew :composeApp:testDebugUnitTest --tests "com.razumly.mvp.eventDetail.TournamentPoolPlayTest" --tests "com.razumly.mvp.eventDetail.DivisionCapacitySummaryTest" --console=plain --no-daemon` and `.\gradlew :composeApp:compileDebugKotlinAndroid --console=plain --no-daemon`.

Revision note, 2026-05-06: Corrected mobile tournament pool-play registration options. Tournament pool-play signup now lists bracket registration divisions from `playoffDivisionDetails`/`PLAYOFF` rows, with a suffix-based fallback that can synthesize bracket options from generated pools while preserving any older bracket-row price/capacity metadata. League playoff signup still lists regular league divisions and filters out playoff rows. Verification passed with `.\gradlew :composeApp:testDebugUnitTest --tests "com.razumly.mvp.eventDetail.EventDetailDivisionOptionsTest" --tests "com.razumly.mvp.core.network.dto.EventDtosTest.event_api_dto_merges_tournament_playoff_division_details_for_registration_pricing" --console=plain --no-daemon`.

Revision note, 2026-05-06: Corrected mobile event-card division labels for tournament pool play. Shared Compose event cards now use event-level display labels that collapse generated pool divisions back to their bracket registration division, while league playoff cards continue to show regular league divisions instead of playoff rows. Verification passed with `.\gradlew :composeApp:testDebugUnitTest --tests "com.razumly.mvp.core.data.util.EventDivisionDisplayTest" --console=plain --no-daemon`.
