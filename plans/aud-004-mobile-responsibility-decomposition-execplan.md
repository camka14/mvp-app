...
- [x] (2026-07-14 22:50Z) Milestone 6: split `BillingRepository` behind `IBillingRepository` into checkout/signing, rental, bill/payment, discount, catalog, organization/review, and refund collaborators. The facade is 585 lines instead of 4,430; all 58 Billing repository HTTP cases and both platform build gates pass at the final integrated checkpoint.
  ...
  - [x] (2026-07-14 22:30Z) Milestone 6e: moved discount observation/refresh, target discovery/cache replacement, discount creation, and discount-code generation/status/deletion into the 277-line `BillingDiscountCoordinator`. `BillingRepository.kt` fell from 1,217 to 1,010 lines. At integrated commit `253dc25f`, all 58 Billing repository HTTP cases, iOS simulator compilation, Android debug assembly, canonical identity handling, and diff checks pass.
  - [x] (2026-07-14 22:41Z) Milestone 6f: moved organization pagination/search/ID reads, tag/template discovery, viewer-scoped Room snapshots, review pagination/fallback, and review mutations/moderation into the 437-line `BillingOrganizationCoordinator`. `BillingRepository.kt` fell from 1,010 to 650 lines, below the facade acceptance target. At integrated commit `ac1dfe25`, all 58 Billing repository HTTP cases, iOS simulator compilation, Android debug assembly, canonical identity handling, and diff checks pass.
  - [x] (2026-07-14 22:50Z) Milestone 6g: moved refund request creation, event deletion/refund ordering, host refund synchronization, user/event hydration, and refund approval/rejection into the 105-line `BillingRefundCoordinator`. `BillingRepository.kt` fell from 650 to 585 lines. At integrated commit `ffa8504b`, all 58 Billing repository HTTP cases, iOS simulator compilation, Android debug assembly, canonical identity handling, and diff checks pass.
...
- [x] (2026-07-15 00:15Z) Milestone 9: finished the component orchestration boundary at 1,467 lines while preserving the component-owned Decompose scope, public interface/state exposure, lifecycle assembly, Room-backed coordinators, and repository contracts.
  - [x] (2026-07-15 00:02Z) Milestone 9a: moved match-edit execution, participant mutations/billing, invite/staff actions, and edit/publish/rental wiring into 220-line, 240-line, 234-line, and 382-line handlers. The component fell from 2,819 to 2,056 lines; the focused coordinator suites and both platform build gates pass at commits `275968e2`, `71ccfffa`, `caee6cd2`, and `bebfe2d7`.
  - [x] (2026-07-15 00:08Z) Milestone 9b: moved registration progress, questions, child selection, membership/withdraw targets, and post-payment confirmation wiring into the 391-line `EventRegistrationLifecycleHandler`. The component fell from 2,056 to 1,723 lines; 63 focused registration/membership/weekly tests and both platform build gates pass at commit `098beccb`.
  - [x] (2026-07-15 00:15Z) Milestone 9c: moved delete/report/share/QR/directions wiring into the 124-line `EventExternalActionHandler`, then division/resource/catalog/template/standings lifecycle wiring into the 236-line `EventResourceLifecycleHandler`. The component fell from 1,723 to 1,467 lines; 38 focused external/lifecycle/division/standings tests and both platform build gates pass at commits `b861cb6b` and `b2e6fa75`. The final Android APK digest is `297ded1b59a4d60e5b5baed63b82ae26e503df3de8d032946dc4306fe3c5b626`.
- [ ] Milestone 10: rerun broad exact-commit validation and complete authenticated Android/iOS event-detail visual smoke before reconciling AUD-004.
  - [x] (2026-07-15 00:31Z) Milestone 10a: completed the final exact-commit broad and runtime gates at `410826d2`. Android passes 1,245 of 1,248 tests with three skipped and zero failures/errors; iOS passes all 1,162 tests. The exact Android APK digest is `297ded1b59a4d60e5b5baed63b82ae26e503df3de8d032946dc4306fe3c5b626`; it installed and cold-launched in 5.779 seconds with no fatal exception, ANR, or process death. The fresh iOS app's code-dylib digest is `c7963ec37f6b5f1df1c6e2f26b4bddb816b936da6816d8d7d771099b8b34564c`; it installed, launched, settled to the coherent login surface, and produced no crash or uncaught termination.
  - [ ] Milestone 10b: complete the authenticated event-detail walkthrough on Android and iOS. The supplied account still returns `Invalid credentials`; this remains blocked until a valid existing test account or an explicitly authorized fixture is available. Credentials and application data will not be changed merely to pass QA.

## Surprises & Discoveries

- Observation: the broad Android suite's only failure was a stale paid-team checkout expectation, not a production regression.
  Evidence: both platform payment processors intentionally return `PaymentResult.Failed("Payment setup is unavailable. Please try again.")` without a native payment-sheet host, and the component clears its pending team-registration purchase state. Updating the integration assertion to that established contract yields 1,234 passing Android tests, three skipped, and zero failures/errors.

- Observation: one Billing HTTP fixture compiled on JVM but not common iOS Kotlin because `MutableMap.getOrDefault` inference was platform-sensitive at that call site.
  Evidence: replacing it with the equivalent common expression `(callsByPath[path] ?: 0)` leaves behavior unchanged and yields 1,151 passing iOS simulator tests with zero skips, failures, or errors.

- Observation: both exact artifacts launch cleanly, but the authenticated runtime walkthrough cannot proceed with the supplied account.
  Evidence: Android cold-launched `com.razumly.mvp/.MainActivity` from the validated APK and remained alive as PID 10892; iOS installed and launched `com.razumly.mvp` on the booted iPhone 17 Pro simulator. Both show coherent login UI and clean app-fatal scans. Submitting `samuelrazumovskiy@razumly.com` with the supplied password returns `Invalid credentials`; credentials and live data were intentionally not mutated during QA.
...
- Observation: the remaining component size came from action and lifecycle wiring around existing state owners, not from another hidden domain model.
  Evidence: seven focused handlers now translate public component actions and lifecycle callbacks into the established edit, participant, invite, registration, membership, resource, and standings coordinators. None owns a duplicate `StateFlow`, Room cache, HTTP response model, or public interface; the component fell from 2,819 to 1,467 lines while every new handler remains 391 lines or smaller.

- Observation: the first fresh iOS app build failed because the build volume ran out of disk space, not because of a source or toolchain defect.
  Evidence: the broad iOS simulator suite had already passed all 1,162 tests. Removing only the incomplete temporary build directory and stale DerivedData caches, then rebuilding through the audit worktree's existing workspace cache, produced a fresh installable app at the exact source commit. No source, simulator data, or user data was deleted, and no code change was needed.
...
- Decision: use action and lifecycle handlers as adapters around existing component coordinators rather than introducing replacement state owners.
  Rationale: the component's remaining bulk was callback construction and repository/platform effect wiring. Keeping mutable policy in the established coordinators preserves singular ownership and Room-first behavior while leaving `DefaultEventDetailComponent` as the Decompose scope, assembly, public-state, and delegation boundary.
  Date/Author: 2026-07-15 / Codex

## Outcomes & Retrospective

Research and sequencing are complete, Milestones 1 through 9 and Milestone 10a are validated, and every structural acceptance target is met. `EventDetailScreen.kt` now owns 1,197 lines instead of 4,137 and meets its structural target. Its detail-tab presentation lifecycle lives in a 522-line callback-driven route host; division/standings derivation lives in a 242-line pure builder; access and weekly-route derivation live in 136-line and 117-line pure builders; and join/withdrawal policy, map state, overview-guide lifecycle, and route effects have focused owners no larger than 193 lines. APP-009 is integrated as `5b862d6d`; event-team check-in lives in a 146-line coordinator, canonical Room-backed relation derivation lives in a 222-line coordinator, participant/bootstrap orchestration lives in a 425-line coordinator, and 22 lifecycle collectors now live behind a 327-line binding owner with six direct regressions. `DefaultEventDetailComponent.kt` now owns 1,467 lines instead of 3,439 and meets its structural target; match, participant, invite, edit, registration lifecycle, external action, and resource lifecycle wiring live in seven focused handlers no larger than 391 lines. The repository contracts/models and event plus billing-domain wire mappings now have explicit files; viewer-scoped cache lifecycle lives in a 77-line coordinator, detail HTTP mechanics in a 129-line gateway, canonical detail cache access in a 30-line Room store, viewer registration cache synchronization in a 130-line coordinator with four direct regressions, participant/relation/management/compliance synchronization in a 699-line coordinator, registration mutations plus their Room convergence in a 473-line coordinator, discovery/catalog reads in a 400-line coordinator, Billing checkout workflows in a 254-line coordinator, Billing signing/onboarding workflows in a 243-line coordinator, rental-order persistence/retry in a 238-line coordinator, bill/payment/subscription workflows in a 439-line coordinator, product catalog/purchase workflows in a 275-line coordinator, discount workflows in a 277-line coordinator, organization/review/template workflows in a 437-line coordinator, and refunds in a 105-line coordinator. `EventRepository.kt` is 1,036 lines instead of 3,398 and `BillingRepository.kt` is 585 lines instead of 4,430; both repository facades meet their structural targets. At exact source commit `410826d2`, Android passes 1,245 of 1,248 tests with three skipped, iOS passes all 1,162 tests, and both exact artifacts install and launch cleanly to coherent login surfaces without an app crash. AUD-004 remains open only for Milestone 10b, the authenticated Android/iOS event-detail visual walkthrough, because the supplied credentials are invalid.
...
`core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRepository.kt` is now a public delegation facade for checkout, signing, rental-order, bill, payment, billing-address, subscription, product catalog, product purchase, discount, organization, review, template, and refund collaborators. `BillingRepositoryContract.kt` and `BillingRepositoryModels.kt` own its stable public surface. Checkout/signing, catalog/discount, rental, and payment wire declarations live in four domain-specific files. Product and organization catalog refreshes retain viewer-scoped atomic DAO replacement and DAO re-read inside their coordinators, while discount and refund persistence ownership live in their dedicated coordinators.
...
Milestone 8 resumes the screen acceptance work from the stable typed render hosts. Map the remaining route body into cohesive presentation-state assembly, dialog/selection ownership, and callback-wiring families. Move only a complete family at a time into a sibling route owner, keep component flow collection and domain mutations singular, and require direct pure-rule coverage for any policy exposed by the move. Do not reduce the file by deleting fields, tabs, actions, or event behavior.

Milestone 9 resumes the component acceptance work from the existing Room relation, participant bootstrap, lifecycle binding, registration, edit, billing, match, and standings coordinators. Extract remaining cohesive state-exposure or delegation families behind source-compatible internal owners. The Decompose component remains the scope and public-interface owner; no new coordinator may duplicate Room state, HTTP state, or an existing workflow coordinator.

Milestone 10 repeats the broad tests and exact-artifact platform build/install/launch checks from the final structural commit. Complete the authenticated walkthrough only with a valid existing local test account or an explicitly authorized fixture; do not change live credentials merely to make the smoke test pass.
...
Milestone 6e integrated evidence on 2026-07-14:

    isolated checkpoint: a249dc70
    integrated checkpoint: 253dc25f
    1,010  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRepository.kt
      277  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingDiscountCoordinator.kt
    BillingRepositoryHttpTest: 58 passed, 0 skipped, 0 failures, 0 errors
    ./gradlew :composeApp:compileKotlinIosSimulatorArm64: BUILD SUCCESSFUL (67 actionable tasks: 12 executed, 55 up-to-date)
    ./gradlew :composeApp:assembleDebug: BUILD SUCCESSFUL (175 actionable tasks: 13 executed, 162 up-to-date)
    composeApp-debug.apk SHA-256: bffd3e2dec8727e5f822129db1a66fb53f793652ac7095117c99e1d416cd570b
    git diff --check: passed

    Owner/item normalization, cached offer/code observation, complete offer and target
    replacement, query-only target behavior, mutation payloads, endpoint encoding,
    Room convergence, and error propagation remain unchanged behind the public facade.

Milestone 6f integrated evidence on 2026-07-14:

    isolated checkpoint: 60145ee5
    integrated checkpoint: ac1dfe25
      650  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRepository.kt
      437  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingOrganizationCoordinator.kt
    BillingRepositoryHttpTest: 58 passed, 0 skipped, 0 failures, 0 errors
    ./gradlew :composeApp:compileKotlinIosSimulatorArm64: BUILD SUCCESSFUL (67 actionable tasks: 12 executed, 55 up-to-date)
    ./gradlew :composeApp:assembleDebug: BUILD SUCCESSFUL (175 actionable tasks: 13 executed, 162 up-to-date)
    composeApp-debug.apk SHA-256: a7c34a48172ff26632234ba2cdc010eea07098b2dd9dea0dcf35a4bb5a1241ae
    git diff --check: passed

    Query normalization, cache-key identity, pagination metadata, offline fallback,
    atomic organization replacement and DAO re-read, review page replacement,
    moderation invalidation, template mapping, and endpoint behavior remain unchanged
    behind the public facade.

Milestone 6g integrated evidence on 2026-07-14:

    isolated checkpoint: ba3aa77c
    integrated checkpoint: ffa8504b
      585  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRepository.kt
      105  core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRefundCoordinator.kt
    BillingRepositoryHttpTest: 58 passed, 0 skipped, 0 failures, 0 errors
    ./gradlew :composeApp:compileKotlinIosSimulatorArm64: BUILD SUCCESSFUL (67 actionable tasks: 12 executed, 55 up-to-date)
    ./gradlew :composeApp:assembleDebug: BUILD SUCCESSFUL (175 actionable tasks: 12 executed, 163 up-to-date)
    composeApp-debug.apk SHA-256: adca4a9dc34b5a052064cca6442893fc989858455c454fb03bef471880bb4a18
    git diff --check: passed

    Canonical refund payloads, server-before-local deletion ordering, host query
    encoding, Room upsert/re-read behavior, best-effort relation hydration, status
    mutations, cache cleanup, logging, and error propagation remain unchanged behind
    the public facade.

Milestone 7 broad and launch evidence on 2026-07-14:

    canonical test corrections: 03880122, fa9b117d
    ./gradlew :composeApp:testDebugUnitTest: BUILD SUCCESSFUL
    Android XML totals: 1,237 tests, 3 skipped, 0 failures, 0 errors
    ./gradlew :composeApp:iosSimulatorArm64Test: BUILD SUCCESSFUL
    iOS XML totals: 1,151 tests, 0 skipped, 0 failures, 0 errors
    composeApp-debug.apk SHA-256: adca4a9dc34b5a052064cca6442893fc989858455c454fb03bef471880bb4a18
    BracketIQ iOS executable SHA-256: 1fa186ffd10fa9893a88a287bf1884f59b9d7388b495c35d8042c67811799ae0
    Android cold launch: com.razumly.mvp/.MainActivity, PID 10892, coherent login UI
    iOS launch: com.razumly.mvp, iPhone 17 Pro simulator, coherent login UI
    app-fatal, AndroidRuntime, uncaught-termination, and app-ANR scans: no finding

    Android briefly displayed a simulator System UI nonresponse dialog; selecting
    Wait returned to the still-running app and the app log contained no crash or ANR.
    The iOS audit worktree uses an ignored symlink to the existing local Secrets.plist
    so no secret was copied or committed. Port 3000 was already owned by an unrelated
    node process and was left untouched. The supplied local test credentials return
    Invalid credentials, so authenticated event-detail visual smoke remains pending.

Milestone 8a evidence on 2026-07-14:

    integrated checkpoint: 841996ab
    1,585  composeApp/src/commonMain/kotlin/com/razumly/mvp/eventDetail/EventDetailScreen.kt
      242  composeApp/src/commonMain/kotlin/com/razumly/mvp/eventDetail/EventDetailDivisionPresentation.kt
    ...
    composeApp-debug.apk SHA-256: a288b5b815510fc451399b35c417fb47d20070672e4ac5e3d0accf38dbd1209b
    git diff --check and git diff --cached --check: passed

    The builder receives immutable event, selection, standings, and feature inputs;
    it has no component, repository, Flow, HTTP, or mutable Compose-state dependency.
...
Revision note (2026-07-14): Completed Milestone 6e by moving discount cache observation/refresh, target discovery, discount creation, and code mutations into `BillingDiscountCoordinator`. Integrated isolated commit `a249dc70` as `253dc25f`, reduced the Billing facade to 1,010 lines, and recorded the 58-case Billing matrix plus Android/iOS artifact evidence. Organizations/reviews are the next boundary, followed by refunds.
Revision note (2026-07-14): Completed Milestone 6f by moving viewer-scoped organization list/search/ID snapshots, tag/template reads, review pagination/fallback, review mutations, and moderation invalidation into `BillingOrganizationCoordinator`. Integrated isolated commit `60145ee5` as `ac1dfe25`, reduced the Billing facade to 650 lines below its acceptance target, and recorded the 58-case Billing matrix plus Android/iOS artifact evidence. Refunds are the final Billing boundary.
...
Revision note (2026-07-15): Completed Milestone 9 by extracting seven action/lifecycle adapters around the existing match, participant, invite, edit, registration, membership, resource, and standings coordinators. Commits `275968e2`, `71ccfffa`, `caee6cd2`, `bebfe2d7`, `098beccb`, `b861cb6b`, and `b2e6fa75` reduce `DefaultEventDetailComponent.kt` from 2,819 to 1,467 lines without changing the public interface or creating duplicate state owners; all focused suites, diff checks, and Android/iOS build gates pass. Milestone 10 now owns final broad exact-commit and runtime validation.
Revision note (2026-07-15): Completed Milestone 10a at exact source commit `410826d2` with 1,248 Android tests (three skipped), 1,162 passing iOS tests, exact-artifact install/cold-launch checks on both platforms, and clean login-surface visual smoke. Recorded the final APK and iOS app-code digests. Milestone 10b and AUD-004 remain open only because the supplied account returns `Invalid credentials`; a valid test account or explicitly authorized fixture is required for the authenticated event-detail walkthrough.
