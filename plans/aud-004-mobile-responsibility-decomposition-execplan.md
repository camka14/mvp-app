...
`core/repository-impl/src/commonMain/kotlin/com/razumly/mvp/core/data/repositories/BillingRepository.kt` contains the implementation for organizations/reviews/refunds and delegates checkout, signing, rental-order, bill, payment, billing-address, subscription, product catalog, product purchase, and discount workflows to focused collaborators. `BillingRepositoryContract.kt` and `BillingRepositoryModels.kt` own its stable public surface. Checkout/signing, catalog/discount, rental, and payment wire declarations live in four domain-specific files. Product catalog refreshes retain viewer-scoped atomic DAO replacement and DAO re-read inside `BillingProductCoordinator`; discount cache ownership now lives in `BillingDiscountCoordinator`; organization refresh paths must preserve the same catalog invariant during their extraction.
