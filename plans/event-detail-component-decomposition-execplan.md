...
- [ ] Thin `DefaultEventDetailComponent` so it owns Decompose lifecycle, public state exposure, and delegation, while helpers own domain-specific transformations.
  ...
  - [x] (2026-06-23) Move registration-progress persistence orchestration and answer-aware registration request branching into `EventRegistrationFlowCoordinator` with focused tests.
  - [x] (2026-06-23) Move event-fullness, membership refresh, withdraw membership, and withdraw-target list decisions into coordinators/helpers with focused tests.
  - [x] (2026-06-23) Move match-edit command gating, commit execution, dialog-state construction, and post-schedule bracket reset reuse into match coordinators/helpers with focused tests.
  - [x] (2026-06-23) Move reschedule, bracket build, and rebuild-without-placeholder execution into an edit-action coordinator with focused tests.
  - [x] (2026-06-23) Move save-event, create-template, publish, and participant billing execution into coordinators with focused tests.
  - [x] (2026-06-23) Move request-refund, withdraw-and-refund, and leave execution into a withdrawal action coordinator with focused tests.
  - [x] (2026-06-23) Move payment-plan bill creation and rollback execution into a billing coordinator with focused tests.
...
The fifty-first implementation milestone extracts league standings side-effect coordination. `EventLeagueStandingsCoordinator.kt` now owns selected-division load handling, manual standings refresh result handling, schedule-refresh target resolution, confirm-standings loading state, confirmation result messages, and post-confirm event/match refresh calls. `DefaultEventDetailComponent` still owns coroutine launching, current event/division lookup, and assigning returned UI messages. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventLeagueStandingsCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` dropped to 5,298 lines after this milestone.
...
The fifty-seventh implementation milestone expands registration-flow coordination. `EventRegistrationFlowCoordinator.kt` now owns registration-progress save/load/clear orchestration through persistence callbacks and chooses the answer-aware or answerless repository callback for self registration, team registration, and purchase-intent creation. `DefaultEventDetailComponent` keeps current progress scope construction, repository wiring, and restored-division assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventRegistrationFlowCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,975 lines after this milestone.

The fifty-eighth implementation milestone batches membership and capacity thinning. `EventOverviewCapacity.kt` now owns event-fullness selection across weekly occurrence summaries, overview summaries, event capacity, and team capacity. `EventMembershipCoordinator.kt` now owns current-user membership refresh orchestration and withdraw-target membership resolution. `EventRegistrationFlowCoordinator.kt` now owns withdraw-target list construction from self/child membership. `DefaultEventDetailComponent` keeps repository wiring, current event/user/occurrence lookup, and state assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventOverviewCapacityTest*" --tests "*EventMembershipCoordinatorTest*" --tests "*EventRegistrationFlowCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,904 lines after this batch milestone.

The fifty-ninth implementation milestone batches match-edit thinning. `EventMatchEditingCoordinator.kt` now owns edit-permission command gating, match-edit dialog state construction, and commit preparation/execution callbacks, while still taking repository and loading behavior as callbacks from `DefaultEventDetailComponent`. `EventMatchEditHelpers.kt` now owns the repeated post-schedule bracket-match reset and refetch sequence used by build/rebuild flows. `DefaultEventDetailComponent` keeps current event/division/relation lookup, coroutine launching, repository wiring, and public action exposure. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventMatchEditingCoordinatorTest*" --tests "*EventMatchEditHelpersTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,876 lines after this batch milestone.

The sixtieth implementation milestone batches schedule-edit action execution. `EventEditActionCoordinator.kt` now owns the common loading, prepare/update, schedule, match-refetch or bracket-reset, standings-refresh, success-message, and failure-result path for reschedule, bracket build, and rebuild-without-placeholder actions. `DefaultEventDetailComponent` keeps public method exposure, repository callback wiring, participant-count and placeholder scheduling arguments, edit-mode cancellation, and UI error assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventEditActionCoordinatorTest*" --tests "*EventMatchEditHelpersTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,838 lines after this batch milestone.

The sixty-first implementation milestone expands edit-action and participant billing coordination. `EventEditActionCoordinator.kt` now owns save-event loading/result flow, staff-assignment previous-user calculation, final staff event update branching, create-template loading/results, and publish loading/results. `EventParticipantManagementCoordinator.kt` now owns participant billing target validation, snapshot loading, bill creation, checkout creation, refund execution, and compliance-refresh-on-success decisions. `DefaultEventDetailComponent` keeps public method exposure, repository callback wiring, current state reads, and UI error/state assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventEditActionCoordinatorTest*" --tests "*EventParticipantManagementCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,815 lines after this batch milestone.

The sixty-second implementation milestone extracts withdrawal action execution. `EventWithdrawalActionCoordinator.kt` now owns request-refund, withdraw-and-refund, and leave execution branching, including weekly occurrence preflight messages, target normalization, membership decision application, team-vs-user repository callback choice, loading labels, refresh-on-success warning selection, failure result mapping, and loading cleanup. `DefaultEventDetailComponent` keeps public method exposure, current event/user/occurrence reads, repository callback wiring, and UI error assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventWithdrawalActionCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,691 lines after this batch milestone.

The sixty-third implementation milestone extracts payment-plan billing execution. `EventPaymentPlanBillingCoordinator.kt` now owns payment-plan bill request validation, selected division payment-plan resolution, weekly occurrence and relative due-date validation, `CreateBillRequest` construction, duplicate-payment-plan success mapping, and user/team rollback callback sequencing with warning logging. `DefaultEventDetailComponent` keeps current event/user/occurrence reads, billing repository wiring, event repository rollback callbacks, and UI error assignment in the larger join flow. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventPaymentPlanBillingCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,645 lines after this batch milestone.

The sixty-fourth implementation milestone extracts purchase-intent handoff coordination. `EventPurchaseIntentCoordinator.kt` now owns purchase-intent required-signature gating, single-document web-signature prompt creation, missing-signature-url warning fallback, registration-hold state and persistence callback dispatch, fee-breakdown pending payment intent setup, and direct payment-sheet launch selection. `DefaultEventDetailComponent` keeps coroutine launching, progress persistence callback wiring, payment processor calls, and UI error assignment. Focused validation passed with `./gradlew :composeApp:testDebugUnitTest --tests "*EventPurchaseIntentCoordinatorTest*" --console=plain`, `./gradlew :composeApp:compileDebugKotlinAndroid --console=plain`, and `git diff --check`. `EventDetailComponent.kt` is 4,611 lines after this batch milestone.
...
Division content coordinator first focused test run caught a Kotlin function-reference issue:

    ./gradlew :composeApp:testDebugUnitTest --tests "*EventDivisionContentCoordinatorTest*" --tests "*EventDetailMobileJoinFlowTest*" --tests "*EventLeagueStandingsCoordinatorTest*" --tests "*LeagueStandingsPresentationTest*"
    Exit code: 1
    Result: `EventDivisionContentCoordinator.kt` used a function reference to a member extension, which Kotlin prohibits.
    Fix: replace the function reference with a lambda call.

Division content coordinator focused tests passed after compile fix:

    ./gradlew :composeApp:testDebugUnitTest --tests "*EventDivisionContentCoordinatorTest*" --tests "*EventDetailMobileJoinFlowTest*" --tests "*EventLeagueStandingsCoordinatorTest*" --tests "*LeagueStandingsPresentationTest*"
    Exit code: 0
    Result: BUILD SUCCESSFUL in 1m 19s; 43 actionable tasks: 9 executed, 34 up-to-date.

Division content coordinator common metadata compilation passed:

    ./gradlew :composeApp:compileCommonMainKotlinMetadata
    Exit code: 0
    Result: BUILD SUCCESSFUL in 10s; 11 actionable tasks: 3 executed, 8 up-to-date.

Forty-fifth milestone line-count evidence:

    5564 composeApp/src/commonMain/kotlin/com/razumly/mvp/eventDetail/EventDetailComponent.kt
      68 composeApp/src/commonMain/kotlin/com/razumly/mvp/eventDetail/EventDivisionContentCoordinator.kt
...
Revision Note (2026-06-23): Recorded the registration-progress and answer-aware request coordinator slice, focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the batched membership/capacity coordinator slice, grouped focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the batched match-edit command and bracket-reset coordinator/helper slice, focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the batched schedule-edit action coordinator slice, focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the batched save/template/publish edit-action and participant-billing coordinator slice, focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the withdrawal action execution coordinator slice, focused tests, compile checks, and line-count impact.
Revision Note (2026-06-23): Recorded the payment-plan billing coordinator slice, focused tests, compile checks, and line-count impact.
