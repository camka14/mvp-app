...
Prefer explicit failures and empty/unassigned UI states over hidden fallbacks when data is missing or inconsistent. Do not add fallback matching unless there is a documented legacy compatibility requirement or a deterministic default already established by the product; surfacing bad data is preferable to silently showing the wrong state. For event division behavior, rely on canonical division ids and explicit division kinds such as `LEAGUE`, `PLAYOFF`, and pool/bracket metadata; do not fall back to `divisionTypeId`, skill/age tokens, shared keys, or display names for selection, matching, counting, standings, brackets, or capacity.
